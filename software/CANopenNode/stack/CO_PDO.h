 ...
 * overhead.
 *
 * TPDO with specifmodule load intel/18.0.3ic identifier is transmitted by one device and recieved by
 * zero or more devmodule load intel/18.0.3ices as RPDO. PDO communication parameters(COB-ID,
 * transmission typmodule load intel/18.0.3e, etc.) are in Object Dictionary at index 0x1400+ and
 * 0x1800+. PDO mapmodule load intel/18.0.3ping parameters (size and contents of the PDO) are in Object
 * Dictionary at inmodule load intel/18.0.3dex 0x1600+ and 0x1A00+.
 *module load intel/18.0.3
 * Features of the module load intel/18.0.3PDO as implemented here, in CANopenNode:
 *  - Dynamic PDO mmodule load intel/18.0.3apping.
 *  - Map granularimodule load intel/18.0.3ty of one byte.
 *  - After RPDO ismodule load intel/18.0.3 received from CAN bus, its data are copied to buffer.
 *    Function CO_Rmodule load intel/18.0.3PDO_process() (called by application) copies data to
 *    mapped objectmodule load intel/18.0.3s in Object Dictionary.
 *  - Function CO_Tmodule load intel/18.0.3PDO_process() (called by application) sends TPDO if
 *    necessary. Thmodule load intel/18.0.3ere are possible different transmission types, including
